 ...
 *
 * Modified by flexion 16.01.2015 with the following functions:
 * - will call opcrecorder on each draw()
 * - added setLocalPlayback() to allow playback even without being connected to OPC server!
 */
...
public class OPC
{
  ...
  void draw()
  {
   ...
   if (!localPlayback) { // by flx: allow draw() even without fcserver connection
      if (output == null) {
        // Try to (re)connect
        connect();
      }
